...
const BlogPage = ({data , blogs , currentPage , pageCount}) => {
    ...
    return (
        <Container sx={{mt: 3}}>
            <Typography variant={'h3'} my={3} fontWeight={'bold'} sx={{display:{xs:'block' , md:'none' }}}>مجله تک جنرال</Typography>
            <Box sx={{px:1}}>
                <Grid container justifyContent={'space-between'} sx={{boxShadow: 1, p: 2, borderRadius: 2}}>
                    <Grid item md={3} xs={12}>
                        <Box sx={{textAlign: 'center', width: '100%' , aspectRatio : '1/1'}}>
                            <Image width={590} height={290} style={{width: '100%', height: 'auto'}} src={blogs[0].main_image.image}
                                   alt={''}/>
                        </Box>
                    </Grid>
                        ...
                        if(index > 0){
                            ...
                            return (
                                <Grid item md={4} xs={12} sx={{p: 1 , '&:hover' : {transform:'translateY(-2%)'} , transition:'all .5s' , minHeight:'360px' }}>
                                    <BlogCart image={item.main_image.image} title={item.title} content={item.content} id={item.id} date={date}/>
                                </Grid>
                            )
                        }
                    })
