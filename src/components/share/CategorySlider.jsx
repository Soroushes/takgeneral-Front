...
const CategorySlider = ({category , categoryUrl})=>{
    return(
                            ...
                            return (
                                <SwiperSlide key={Math.random() * 1000} style={{width: 150, padding: '50px 2px'}}>
                                    <Link scroll={false} href={'/category/'+item.url}>
                                        <OuterImageSection isActive={categoryUrl === item.url} descriptionVariant={'subtitle2'} titleVariant={'h6'} title={item.name} image={item.image}/>
                                    </Link>
                                </SwiperSlide>
                            )
                        })
